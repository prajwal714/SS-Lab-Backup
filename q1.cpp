...
class task{
	public:
		...
		task(int id,int bt,int ct=0,int isScheduled=false, int at=0)
		{
			this->id=id;
			this->at=at;
			this->bt=bt;
...
//			time+=timeq;
	...
	for(i=0;i<n;i++)
	{
		avgTurnAroundTime+=t[i].ct-t[i].at;
		avgWaiting+=t[i].ct-t[i].at-t[i].bt;
		cout<<i<<"|\t"<<t[i].at<<"\t|\t"<<t[i].bt<<"\t|\t"<<t[i].ct<<"\t|\t"<<t[i].ct-t[i].at<<"\t|\t"<<t[i].ct-t[i].at-t[i].bt<<endl;
	}
	
	cout<<"Avg waiting time : "<<avgWaiting/n<<endl;
	cout<<"Avg Turn around time: "<<avgTurnAroundTime/n<<endl;
